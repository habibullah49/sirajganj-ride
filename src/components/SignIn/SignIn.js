    ...
    return (
        <div className="form-signin mt-5">
            <form onSubmit={handleSubmit}>
                ...
                <button className="w-100 btn btn-lg btn-outline-danger mb-2" type="submit">{newUser ? 'Create an account' : 'Signin'}</button>
                <p>{newUser ? 'Already have an account' : 'Don’t have an account'} ?
                 <span onClick={() => setNewUser(!newUser)} className="text-success" style={{ cursor: 'pointer' }}>
                        {newUser ? ' Signin' : ' Create an account'}
                    </span></p>
                <p>Or</p>
                <hr />
                <button onClick={handleGoogleSignIn} className="w-100 btn btn-lg btn-outline-danger mb-2" type="submit">SignIn with Google</button>
